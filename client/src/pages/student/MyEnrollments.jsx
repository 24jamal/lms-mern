    ...
    const getCourseProgress = async () => {
        try {
            ...
            const tempProgressArray = await Promise.all(enrolledCourses.map(async (course) => {
                ...
                const { data } = await axios.post(`${backendUrl}/api/user/get-user-progress`, { courseId: course._id }, { headers: { Authorization: `Bearer ${token}` } })
                            ...
                            <tr key={index} className='border-b border-gray-500/20'>
                                <td className='md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3'>
                                    ...
                                    <div className='flex-1'>
                                        ...
                                        <Line strokeWidth={2} percent={progressArray[index] ? (progressArray[index].lectureCompleted * 100) / progressArray[index].totalLectures : 0} className='bg-gray-500 rounded-full' />
                                    </div>
                                </td>
